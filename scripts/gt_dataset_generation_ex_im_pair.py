...
with open("../datasets/gt_rt_exim_small_sample.csv", 'wt') as out_file:
    ...
    for task_folder in os.listdir(ROOT):
        ...
        for instance_folder in os.listdir(os.path.join(ROOT, task_folder)):
            ...
            with open(os.path.join(ROOT, task_folder, instance_folder, 'natural_language.txt'), 'r') as f:
                ...
                elif nl_type == 'implicit':
                    im_sentences_list.append(line)

        sentences_pair = []
        if len(ex_sentences_list) == 0:
            for i in range(min(len(im_sentences_list), 500)):
                for j in range(i+1, min(len(im_sentences_list), 500)):
                    sentence1 = im_sentences_list[i]
                    sentence2 = im_sentences_list[j]

                    if sentence1 != sentence2:
                        sentences_pair.append([sentence1, sentence2])
        elif len(im_sentences_list) == 0:
            for i in range(min(len(ex_sentences_list), 500)):
                for j in range(i+1, min(len(ex_sentences_list), 500)):
            ...
            if len(ex_sentences_list) > 500:
                random.shuffle(ex_sentences_list)
                ex_sentences_list = ex_sentences_list[:500]
            if len(im_sentences_list) > 500:
                random.shuffle(im_sentences_list)
                im_sentences_list = im_sentences_list[:500]

            for i in range(len(ex_sentences_list)):
                for j in range(len(im_sentences_list)):
                    sentence1 = ex_sentences_list[i]
                    sentence2 = im_sentences_list[j]
